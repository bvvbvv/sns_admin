    ...
    if ($row) {
        if ($row instanceof mysqli_result && $row->num_rows > 0) {
            while ($r = $row->fetch_assoc()) {
                ...
                $proc_t=$proc_bank+$proc_ekvar;  // Итоговая сумма для реселлера только по безналу
                $proc_t=round($proc_t,2);  // Итоговая сумма для реселлера только по безналу
                $cells[]=$proc_t;  // Итоговая сумма для реселлера только по безналу
