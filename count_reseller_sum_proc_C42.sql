...
create temporary table `rtemp` (`rid` int not null, res_name char(128) character set utf8, `sys_name` char(20), `cnt_bank` int(11),`sum_bank` DECIMAL(10,2),
...
OPEN cur1;
read_loop: LOOP
FETCH cur1 INTO rid,res_name, sys_name;
    IF done THEN
      LEAVE read_loop;
    END IF;
CALL C32(rid,dat_beg,dat_end, cnt_bank,sum_bank, cnt_local, sum_local, cnt_ekvar, sum_ekvar, cnt_tot, tot_sum);
insert into rtemp values(rid,res_name,sys_name, cnt_bank,sum_bank, cnt_local, sum_local, cnt_ekvar, sum_ekvar, cnt_tot, tot_sum);
END LOOP;
CLOSE cur1;
